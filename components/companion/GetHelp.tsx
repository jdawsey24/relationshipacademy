...
export default function GetHelp() {
  ...
  return (
    <>
      ...
      {open && <HelpDialog resources={resources} onClose={() => setOpen(false)} />}
    </>
  );
}

function HelpDialog({ resources, onClose }: { resources: SafetyResource[]; onClose: () => void }) {
  ...
  return (
    <div ref={ref} tabIndex={-1} className="fixed inset-0 z-50 overflow-y-auto bg-warm-ivory outline-none" role="dialog" aria-modal="true" aria-labelledby="gethelp-title">
      <div className="mx-auto max-w-md px-6 py-12 text-center">
        ...
        <p className="mt-3 font-body text-body text-charcoal/70">The Relationship Companion is an educational tool — not a crisis or therapy service. If you&apos;re in distress, please reach out to one of these.</p>
        <ResourceList resources={resources} />
        <button onClick={onClose} className="mt-6 w-full rounded-full bg-midnight-navy py-3.5 font-ui text-sm font-semibold text-white">Close</button>
      </div>
    </div>
  );
}
