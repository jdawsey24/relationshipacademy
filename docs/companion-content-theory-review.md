...
## 🟡 Notes — theory fidelity (generalized claims stated as fact) — ✅ RESOLVED (2026-07-21)
All five re-authored (owner-approved) + re-published via the governance ladder (v6). Absolute/empirical assertions softened to reflective, non-asserting framing ("usually means"→"it can help to ask", "one of the strongest signs"→"a meaningful part of how a relationship holds over time", "unspoken ones are [a dealbreaker]"→"what tends to make them harder", "usually about fairness"→"often… for many people"), so the framework isn't implicitly expanded. `we-are-blending-families` regrounded from "united front" to a Coordination framing ("getting on the same page / staying coordinated as a couple").
