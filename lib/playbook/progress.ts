...
//
// R3: CURRENT-STATE persistence only. The append-only playbook_events log is designed
// but not built — saveProgress is the single write path where a future event emit
// would attach without reshaping this table.
...
export async function loadProgress(
  ...
  const { data } = await admin
    ...
    .select("playbook_key, playbook_version, recognized, play_states, outputs, my_plays")
    .eq("user_id", userId)
