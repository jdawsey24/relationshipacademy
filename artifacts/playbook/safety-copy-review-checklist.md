...
**55 items · none clinically reviewed · all publish-held.** Generated 2026-07-31 from the integrated corpus (`content/playbook/`).

Every piece of safety/support copy across the 30 Playbooks + add-ons, grouped by clinical priority. Work top to bottom. For each: read the copy, tick the box, and record a decision (✅ approve / ✏️ revise → new wording / 🗑️ remove). **Shared items appear once** — a decision there applies to every Playbook that uses it (see *Appears in*).

Safety copy lives in three schema locations: play-level `supportSignposts`, top-level `recognitionCards` (role `signpost`), and `literature` entries. The `id` is the authoritative anchor — grep the codebase by id to find the source. The priority grouping is a suggested order, not a clinical judgement — trust your own read over it.

> Working document — edit the checkboxes and add decisions inline. Nothing ships until this is cleared.

---

## P1 · Crisis & mental-health escalation  (15)

Copy that names suicidal thinking, hopelessness, or a feeling "bigger/heavier than a tool," and escalates to a GP, therapist, mental-health professional, crisis line, or helpline. **Highest clinical + liability priority — review first.** Confirm the thresholds, escalation language, and referral routing are clinically correct and appropriately worded.

### 1. `signpost-c12-not-coping`

- [ ] Reviewed
- *Type:* in-play support signpost · play `the-shape-of-a-week` · in `letting-go`
...
- [ ] Reviewed
- *Type:* in-play support signpost · play `what-it-actually-means` · in `moving-beyond-rejection`
...
### 3. `signpost-c11-flatness`

- [ ] Reviewed
- *Type:* in-play support signpost · play `which-way-does-it-lean` · in `accepting-what-is`
...
### 5. `signpost-c16-intrusive`

- [ ] Reviewed
- *Type:* in-play support signpost · play `the-checking` · in `rebuilding-trust`
...
### 6. `signpost-c23-alone`

- [ ] Reviewed
- *Type:* in-play support signpost · play `telling-one-person` · in `making-confident-decisions`
...
### 7. `signpost-c25-the-why`

- [ ] Reviewed
- *Type:* in-play support signpost · play `the-recurring-move` · in `from-the-ground-up`
...
### 8. `signpost-c3-depleted`

- [ ] Reviewed
- *Type:* in-play support signpost · play `when-closeness-costs` · in `letting-someone-in`
...
### 9. `signpost-c3-heavier-material`

- [ ] Reviewed
- *Type:* in-play support signpost · play `one-true-thing` · in `letting-someone-in`
...
### 10. `signpost-c5-cant-tell-anything`
...
### 11. `signpost-c6-consumed`

- [ ] Reviewed
- *Type:* in-play support signpost · play `what-this-is-costing` · in `finding-security`
...
### 12. `signpost-c6-worth-contingent`

- [ ] Reviewed
- *Type:* in-play support signpost · play `when-it-doesnt-land` · in `finding-security`
...
### 13. `signpost-c8-self-disconnection`

- [ ] Reviewed
- *Type:* in-play support signpost · play `have-i-stopped-reaching` · in `finding-your-way-back`
...
### 14. `signpost-generalised-hopelessness`

- [ ] Reviewed
- *Type:* in-play support signpost · play `them-or-the-pattern` · in `dating-without-losing-hope`
...
### 15. `rec-c9-rule-out-first`

- [ ] Reviewed
- *Type:* recognition card (signpost) · in `rebuilding-physical-connection`
...
## P2 · Safety, abuse & control (incl. the shared safety guide)  (9)

The "if you don't feel safe" family — DV, coercion, financial control, harm, addiction. The shared card + guide cover **3 Playbooks at once** — the highest-value single review (README).
...
### 20. `signpost-c3-mistreatment`

- [ ] Reviewed
- *Type:* in-play support signpost · play `before-you-go` · in `letting-someone-in`
...
## P3 · Bereavement, grief & loss  (17)

The add-on grief material the README flags as sensitive. Review for tone, accuracy, and that it never oversteps into specialist grief / fertility-loss territory it isn't scoped for.

### 25. `lit-bereave-changes`
...
>
> **What people expect, and what happens:** Expected: a gradual decline. Bad, then less bad, then manageable.
>
...
>
> **Replacing and adding are different:** Replacement assumes a single slot — that a new person would occupy the place the last one had, and therefore displace them.
>
...
>
> **Two questions that are more useful than readiness:** Would I be able to talk about them? Not hide the marriage, not manage around it. If the answer is no, that's worth knowing before rather than after.
>
...
>
> **What the guilt is doing:** Grief is one of the few remaining forms of contact. While it's acute, they're still present in the day.
>
...
### 29. `lit-bereave-nobody-understands`
...
>
> **It's substantially accurate:** The available scripts are built for divorce. People offer them because they're what exists — you'll be better off, you'll find someone, it takes about a year.
>
...
>
> **Two different things being asked:** What's true: whether you are still married, still a we, still someone's. Those aren't questions with clean answers and they don't have to be resolved.
>
...
>
> **The thing that runs through most of this:** Not: how do I get over it. Very few people here are asking that.
>
...
>
> This is one of the least acknowledged losses there is, because nothing happened to it. There's no event, no date, nothing anyone can point at.
>
...
>
> **Why it's harder to grieve than the relationship:** Nobody else can see it. There's no version of it that friends witnessed, so there's nothing to commiserate about.
>
> And it feels illegitimate — you're mourning something that never existed, which sounds like sentimentality rather than loss.
>
...
### 33. `lit-grievediff-different-now`
...
>
> **Worth separating, because they don't move together:** The grief changes on its own timescale and doesn't respond to effort.
>
...
>
> **Why the usual approach stalls:** Most people support someone the way they'd want to be supported. If you need to talk, you ask questions. If you need quiet, you give space.
>
...
### 35. `lit-grievediff-the-question`
...
>
> **The two costs of it:** The moment itself — being asked, and having to produce something.
>
...
>
> **What each usually concludes about the other:** The one who talks about it hears the silence as not caring, or as having moved on.
>
...
### 37. `lit-grievediff-what-this-is`
...
>
> **Two different weights:** The grief. Yours, and it isn't something a relationship guide should be handling. Fertility counsellors and pregnancy loss services exist precisely for this and they are considerably better at it than anything written generally.
>
...
## P4 · Other in-play support signposts  (4)

Play-level "notice this / it's common" support cards that don't escalate to a professional. Still your clinical voice — confirm each is accurate and doesn't minimise something that needs escalation.

### 42. `signpost-c20-practical`
...
### 43. `signpost-c7-always-repairing`
...
"This Playbook isn't the right fit — look for one about X" recognition cards. Low crisis risk; review mainly for redirect accuracy and that nothing minimises a real concern.

### 46. `rec-c23-expectations`
