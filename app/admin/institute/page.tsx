...
export default function AdminInstitute() {
  ...
  const [section, setSection] = useState<InstituteSectionKey>("ce_courses");
  ...
  const visible = useMemo(() => rows.filter((r) => r.section === section), [rows, section]);
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-charcoal/60">
        Manage the offerings shown on each Institute section page. If a section has no offerings here, the site shows built-in defaults.
      </p>
      ...
      <div className="mb-6 flex flex-wrap gap-1 border-b border-light-gray">
        {INSTITUTE_SECTIONS.map((s) => (
          <button key={s.key} type="button" onClick={() => setSection(s.key)}
            className={`-mb-px border-b-2 px-4 py-2 text-sm ${section === s.key ? "border-midnight-navy font-semibold text-midnight-navy" : "border-transparent text-charcoal/60 hover:text-charcoal"}`}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      {canWrite && (
        <div className="mb-6 rounded-lg border border-light-gray p-4">
          <p className="mb-2 text-sm font-medium text-charcoal">Add offering to {INSTITUTE_SECTIONS.find((s) => s.key === section)?.label}</p>
