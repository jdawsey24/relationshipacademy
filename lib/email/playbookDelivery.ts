...
export async function sendPlaybookDeliveryEmail(opts: {
  to: string;
  clusterId: number;
  /** Guest purchase: one-time link to set a password on the account we just made. */
  ...
  try {
    ...
    // A guest bought before having an account, so the button has to finish setting
    // it up — the Playbook is already granted and waiting on the other side.
    const guest = Boolean(opts.setPasswordLink);
    const url = opts.setPasswordLink || `${SITE}/playbook/${key}`;
    const cta = guest ? "Set My Password &amp; Open It" : "Open My Playbook";
    const ctaText = guest ? "Choose a password and open it here" : "Open it here";
    const setupLine = guest
      ? `<tr><td style="font-size:17px;line-height:1.65;color:${CHARCOAL};padding:6px 0;">One quick step first: choose a password below and your Playbook will be waiting in your library.</td></tr>`
      : "";
    const setupText = guest
      ? "\n\nOne quick step first: choose a password and your Playbook will be waiting in your library."
      : "";
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${IVORY};padding:28px 12px;"><tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:16px;padding:36px 32px;font-family:Georgia,serif;color:${CHARCOAL};">
      ...
      ${setupLine}
      <tr><td style="padding:14px 0 4px;"><a href="${url}" style="display:inline-block;background:${CORAL};color:#fff;text-decoration:none;font-weight:600;font-size:16px;padding:14px 30px;border-radius:9999px;">${cta}</a></td></tr>
