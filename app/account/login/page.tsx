...
// Neutral account sign-in — the shared identity behind purchases (Playbooks, etc.).
// Deliberately NOT branded "Academy": having an account is not the same as joining
// the Academy. Honors a same-origin ?next= so a buyer returns to finish checkout.
export default function AccountLoginPage() {
  ...
  // Read the query string client-side after mount so the SSR and first client
  // render agree (avoids a hydration mismatch on the ?next=-carrying links).
  const [nextParam, setNextParam] = useState("");
  useEffect(() => { setNextParam(window.location.search); }, []);

  // Only accept a relative path ("/…", not "//…") to avoid open-redirects.
    ...
    <div className="flex min-h-screen items-center justify-center px-6 py-16">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 font-body text-sm text-charcoal/70">Sign in to your account</p>
        </div>
        <form onSubmit={handleSubmit} className="rounded-2xl border border-midnight-navy/10 bg-white p-8 shadow-sm">
          ...
          <div className="mt-4 flex items-center justify-between font-ui text-sm">
            ...
            <Link href={`/account/signup${nextParam}`} className="text-midnight-navy/70 hover:text-midnight-navy">Create account</Link>
          </div>
        </form>
      </div>
    </div>
